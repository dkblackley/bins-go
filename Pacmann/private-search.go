...
func PacmannMain(args globals.Args) PIRGraphInfo {
	...
	//rtt := args.RTT
	// nonPrivate := false // Debug mode - I don't think this'll work anymore!
	...
	if inputFile == "" {
		log.Fatalf("No input file specified. If you want to use synthetic data, use -input synthetic instead.")
	}
	...
	queryEngine := PIRGraphInfo{
		...
		graph:          graph,
		vectors:        vectors,
		skipPrep:       false, // if benchmarking, we will skip PIR prep
...
//func (queryEngine PIRGraphInfo) DoSearch(frontend graphann.GraphANNFrontend, queryIdx int) ([]int, error) {
//
//	stepN := 15
//	parallelN := pianopir.ThreadNum
//	benchmarking := false
...
//		if err != nil {
//			log.Fatalf("Error creating the report file: %v", err)
//		}
//
...
func (g PIRGraphInfo) Preprocess() {
	...
	for i := 0; i < N; i++ {
		...
		// we then copy the entry to the rawDBx
		rawDB[i] = entry
	}
...
// Entry is what is taken taken from the PIR
